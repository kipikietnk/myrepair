...
import type { MessageContent, GeminiResponse } from "./core/gemini.js";
...
function formatMessage(markdown: string): string {
  ...
  html = html.replace(REGEX_PATTERNS.table, (match) => {
    ...
    if (isHeaderSeparator) return '<!--SEPARATOR-->';
...
function addToHistory(content: MessageContent): void {
...
function buildDiagramPrompt(data: any[]): string {
  return data
    .map(item => {
