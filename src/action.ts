...
function initPanzoom() {
  ...
  try {
    if (state.panzoomInstance) {
      (state.panzoomInstance as any).dispose();
    }
    ...
    elements.imageWrapper.addEventListener('dblclick', utils.throttle((e) => {
      ...
      e.preventDefault();
      const currentTransform = (state.panzoomInstance as any).getTransform();
...
class FollowCursorDrag {
  ...
  public pointerStart: { x: number; y: number };
  public dragMoved: boolean;
  public suppressClickFor: HTMLElement | null;
  public DRAG_THRESHOLD: number;
  public longPressTimer: ReturnType<typeof setTimeout> | null;
  public LONG_PRESS_DELAY: number;
  ...
  init() {
    ...
    document.addEventListener('click', (e) => {
      ...
      const t = (e.target as any).closest('.draggable');
  ...
  startDrag(e: any, element: any, fromLongPress = false) {
